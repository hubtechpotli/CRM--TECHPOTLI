...
export function PremiumDataTable<T extends Record<string, unknown>>({
  ...
  return (
    <div>
      <div ref={parentRef} className={cn("overflow-x-auto", useVirtual && "max-h-[min(70vh,640px)] overflow-y-auto")}>
        <table className="w-full min-w-[640px] text-left text-sm">
          <thead className={useVirtual ? "sticky top-0 z-10 bg-card" : undefined}>
          ...
          <tbody>
            {useVirtual && virtualItems.length > 0 ? (
              <tr>
                <td colSpan={colSpan} style={{ height: virtualItems[0].start, padding: 0, border: 0 }} />
              </tr>
            ) : null}
            {useVirtual
              ? virtualItems.map((vRow) => renderRow(rows[vRow.index], vRow.index))
